...
acq='acq-begin';
files=dir(fullfile(video_incom, acq, '*.MP4'));
    for j=1:length(files)
        filename_n=sprintf('%s_automaticity_%s_rec-%.2d_video.mp4', sub,  acq, j);
...
if length(files)>1 %contains directory ., .., acq-mobile, acq-end, acq-begin
...
for i=1:length(rec_nirs)
    for e=1:length(ext)
        ...
        filename_n=sprintf('%s_automaticity_acq-online_rec-%.2d_nirs.%s', sub, rec_nirs(i), ext{e});
        file=dir(fullfile(nirs_incom, filename_o));
        if length(file)==1
            [success, message]=movefile(fullfile(file.folder, file.name), fullfile(nirs_outgo, filename_n));
            if success
...
if nirs_offline
  ...
  for a=1:2
    ...
    for i=1:length(rec_nirs)
      ...
      filename_n=sprintf('%s_automaticity_%s_rec-%.2d_nirs.oxy3', sub, acq{a}, rec_nirs(i));
...
if length(files)>6 %contains directory . and .. (x3); and acq-24065, acq-24068
...
for i=1:length(rec_motion)
    for e=1:length(ext)
        ...
        filename_n=sprintf('%s_automaticity_run-%.2d_motion.%s', sub, run_motion(i), ext{e});
...
if length(files)>1 %contains directory . and ..
    warning('Remaining files in incoming directory')
else
    fprintf('No remaining files in incoming directory \n\n');
end
