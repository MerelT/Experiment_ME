...
for h = 1:N_trials
     ...
     if str2num(letters_handnonautodual(h).reported_G)==sum(strcmp(letters_handnonautodual(h).presented, 'G'))
      ...
      delay=mean(diff((presses_handnonautodual(h).secs))-1/1.50);
...
%       end
      if strcmp(presses_handnonautodual(h).key,sequencenonautoprint)
          fprintf('Seq correct \n')
      else
...
for e = 1:N_trials
    fprintf('Trial %d: \n', e)
        margin=0.25; % margin of error: think about what is most convenient
      delay=mean(diff((presses_handnonauto(h).secs))-1/1.50);
      fprintf('the tempo was off with on average %f seconds \n', delay);
      if strcmp(presses_handnonauto(h).key,sequencenonautoprint)
          fprintf('Seq correct \n')
      else
          fprintf('Seq incorrect \n')
      end         
...
for f = 1:N_trials
    fprintf('Trial %d: \n', f)
        margin=0.25; % margin of error: think about what is most convenient
      delay=mean(diff((presses_handauto(h).secs))-1/1.50);
      fprintf('the tempo was off with on average %f seconds \n', delay);
      if strcmp(presses_handauto(h).key,sequenceautoprint)
          fprintf('Seq correct \n')
      else
          fprintf('Seq incorrect \n')
      end         
